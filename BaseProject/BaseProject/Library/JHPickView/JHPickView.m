...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.bgV.backgroundColor = RGB(245, 245, 245);
...
//        [self showAnimation];
        ...
        [self.cancelBtn setTitleColor:RGB(55, 159, 242) forState:UIControlStateNormal];
        //完成
        ...
        [self.conpleteBtn setTitleColor:RGB(55, 159, 242) forState:UIControlStateNormal];
        ...
        [line mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.mas_equalTo(self.cancelBtn.mas_bottom).offset(0);
...
- (NSDateFormatter *)formatter {
    ...
    [_formatter setDateFormat:@"dd日MM月yyyy年"];
    return _formatter;
...
//        UIDatePickerModeDateUIDatePickerModeDate
...
//        [self.datePicker setTintColor:[UIColor redColor]];//设置主色
    ...
    switch (arrayType) {
        ...
        case ProjectArray:
        {
            self.selectLb.text = @"请选择报单项目";
            [self.array addObject:self.projectMarr];
//            [self.array addObject:@[@"男",@"女"]];
        }
            break;
        default:
...
//        label.adjustsFontSizeToFitWidth = YES;
        ...
        [label setFont:[UIFont systemFontOfSize:kfont]];
...
{
    for(UIView *speartorView in self.pickerV.subviews)
    {
        if (speartorView.frame.size.height < 1)//取出分割线view
        {
            speartorView.backgroundColor = RGB(209, 209, 209);//隐藏分割线
        }
    }
}
