...
-(UITextField *)passwor{
    ...
    if (!_passwor) {
        ...
        self.passwor = [[UITextField alloc]initWithFrame:CGRectMake(80, 0, screenWidth - 120, 48)];
        self.passwor.textColor = [UIColor colorWithHexString:@"#333333"];
        self.passwor.placeholder = @"6-12位字母／数字" ;
...
-(UITextField *)surePassword{
    ...
    if (!_surePassword) {
        ...
        self.surePassword = [[UITextField alloc]initWithFrame:CGRectMake(90, 0, screenWidth - 120, 48)];
...
-(void)surePress{
    ...
    if (![self.passwor.text isEqualToString:@""] && ![self.surePassword.text isEqualToString:@""]) {
        if ([self.passwor.text isEqualToString:self.surePassword.text]) {
            
            if (self.passwor.text.length >= 6 || self.surePassword.text.length >= 6) {
                NSDictionary *dic = @{
                                      @"newPwd":self.passwor.text,
                ...
                [ConfigModel mbProgressHUD:@"密码不能少于6位" andView:nil];
            }
            
        } else {
            [ConfigModel mbProgressHUD:@"密码两次输入不同" andView:nil];
        }

    } else {
      [ConfigModel mbProgressHUD:@"密码不能为空" andView:nil];
    }
